...
def main(conf : DictConfig) -> None:
    ...
    class SliarEnvironment:
        ...
        def step(self, action):
            ...
            # cost = PI + Qu^2 // P = 1, Q = 10e-06, R = 1E-6
    ...
    ax1.plot(range(max_t+1), states[:,0].flatten(), '.-', label = 'S')
    ax1.legend(loc='upper left')
    ax2 = ax1.twinx()
    ax2.plot(range(max_t+1), states[:,1].flatten(), '.-', label = 'L')
    ax2.plot(range(max_t+1), states[:,2].flatten(), '.-', label = 'I')
    ax2.plot(range(max_t+1), states[:,3].flatten(), '.-', label = 'A')
    ...
    plt.xlabel('day')
    plt.savefig('SLIAR_control.png', dpi=300)
    plt.show(block=False)
